...
export type NewPayerPageParams = { params: Promise<{ tenantSlug: string }>; searchParams: Promise<{ error?: string }> };

/** Conteúdo puro — ver comentário equivalente em `contracts/new/NewContractForm.tsx`. */
export async function NewPayerFormContent({ params, searchParams }: NewPayerPageParams) {
  ...
  if (!canWrite) {
    return <p className="text-body text-content-muted">Sem permissão para criar pagadores neste tenant.</p>;
  }
