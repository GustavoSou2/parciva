...
export type StatementLinePageParams = {
 ...
 * Aqui retorna DOIS blocos (contexto da linha + formulário), então quem
 * chama decide se cada um vira um `Card` (página cheia) ou fica direto
 * dentro do `Modal` (que já é o cartão).
 */
export async function StatementLineFormContent({ params, searchParams }: StatementLinePageParams) {
  ...
  if (!canWrite) {
    return {
      context,
      form: <p className="text-body text-content-muted">Sem permissão para registrar pagamentos neste tenant.</p>,
    };
  }

  const form = !payerIdParam ? (
    payers.length === 0 ? (
